    ...
    <header className="festive-header sticky top-0 z-40 border-b border-border/70 bg-card/90 shadow-[0_18px_45px_rgba(0,0,0,0.32)] backdrop-blur-xl">
      ...
      <div className="container relative mx-auto px-4 py-4">
        <div className="flex items-start justify-between gap-3 sm:items-center sm:gap-4">
          <div
            className="flex min-w-0 flex-1 cursor-pointer items-center gap-3 transition-opacity hover:opacity-90 md:gap-4"
            onClick={async () => {
              await pulseLoading("Feliz Dia das Maes", 1000);
              setLocation("/");
            }}
          >
            <div className="festive-logo-frame flex-shrink-0 rounded-[1.6rem] border border-white/10 bg-black/25 p-3 shadow-[0_0_24px_rgba(255,124,17,0.18)] md:p-3">
              <img
                src={restaurantLogo}
                alt={restaurantName}
                className="h-16 w-16 object-contain md:h-16 md:w-16 lg:h-[4.35rem] lg:w-[4.35rem]"
              />
            </div>

            <div className="flex-1 min-w-0">
              <div className="mb-2 flex flex-wrap items-center gap-2">
                <span className="festive-chip rounded-full px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.32em] text-accent">
                  Brasa acesa
                </span>
              ...
              <h1 className="max-w-[14ch] text-[1.35rem] font-black leading-[0.95] text-foreground sm:max-w-none sm:text-[1.9rem] md:text-[2.35rem]">
                {restaurantName}
              </h1>
              {subtitle && (
          ...
          {showCart && (
            <Button
              variant="outline"
              size="lg"
              className="festive-cart relative mt-1 flex-shrink-0 border-white/15 bg-black/20 text-foreground hover:bg-accent hover:text-accent-foreground sm:mt-0"
              onClick={async () => {
